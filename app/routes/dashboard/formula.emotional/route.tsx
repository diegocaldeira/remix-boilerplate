...
export const meta: MetaFunction = mergeMeta(() => {
  return [{ title: "Gerar Conteúdo com a Fórmula EMOTIONAL_TRIGGERS" }]
})

export const loader = async ({ params }: LoaderFunctionArgs) => {
  let feature = await getFeatureByKeyname("EMOTIONAL_TRIGGERS")
  feature = feature[0]
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  console.log("generating content with formula EMOTIONAL_TRIGGERS")
  ...
  const { brandname, description, trust } = submission.value
  ...
  const command = new StartExecutionCommand({
    ...
    input: JSON.stringify({
      event_type: "process-copywriting-emotional",
      timestamp: Date.now().toString(),
      data: { brand_name: brandname, description, trust },
    }),
  })
...
const schema = z.object({
  ...
  trust: z.string().min(8, "A ação deve ter pelo menos 8 caracteres"),
})
...
export default function FormulaPage() {
  ...
  const [form, { brandname, description, trust }] = useForm({
    id,
    lastSubmission,
                ...
                <div className="w-full space-y-6">
                  ...
                  <div>
                    <Label htmlFor="description">
                      Descreva o que sua marca/empresa faz? (Em 5 a 6 palavras)
                    </Label>
                    <div className="mt-2">
                      <Input
                  ...
                  <div>
                    <div className="flex items-center justify-between">
                      <Label htmlFor="trust">
                        Construir confiança ou confiabilidade
                      </Label>
                    </div>
                    <div className="mt-2">
                      <Input
                        ...
                        id="trust"
                        type="text"
                        placeholder="Depoimentos de clientes satisfeitos..., Nossa garantia de que..."
                        {...conform.input(trust, { type: "text" })}
                      />
                    </div>
                  </div>
                </div>
